...
const COVER = [
  ...
  ['... and axial load on beam', 'L5', 'APPROXIMATED', 'N_Ed = ' + constants.N_AX + ' kN compression with U_x at End 1 - the ULS DESIGN value: the tool\'s axial input is N_Ed entered directly and not run through the combinations (index.html "Axial F, kN"), so in MasterSeries enter an axial load whose factored value is ' + constants.N_AX + ' kN (' + (constants.N_AX / constants.GAMMA_Q) + ' kN imposed x ' + constants.GAMMA_Q + '). Checks: N_c,Rd (A_eff where the web is Class 4 in compression), M-N cross-section 6.2.9, flexural buckling with the strut lengths from the end fixities, Annex B 6.61 / 6.62 with the LTB M_b.Rd, PFC torsional-flexural buckling 6.3.1.4 - all evaluated and printed. Since 20 Sep 2026 (torsion + N/Mz) the combination with the torsion is verified as Eurocode advises: the elastic yield criterion EN 1993-1-1 6.2.7(5)/(6.1) with N/A + M_y/W_el,y + M_z,tot/W_el,z + sigma_w and tau_V + tau_t + tau_w at every station and section point (verdict-binding for an open section with N_Ed, since no expression of EN 1993-1-1 or P385 combines N_Ed with the bimoment plastically; information only for the Class 1/2 hollow sections, whose verdict rests on the code plastic route T_Ed/T_Rd, V_pl,T,Rd, 6.2.8(4) and 6.2.9.1 - 20 Sep 2026 review), Eq 6.61/6.62 with M_z,Ed = M_z + phi.M_y, EN 1993-6 (A.1) with M_z,tot; the former "not implemented as one interaction" block is gone (no Eurocode expression combines N_Ed with warping torsion at member level - the basis is printed, with an information-only superposition of Eq 6.62 and (A.1)).'],
  ['load applied at angle on beam', 'L6', 'APPROXIMATED', 'The tool has no lateral (y-direction) load. A further 30 kN point load at 2.0 m inclined ' + constants.INC_DEG + ' deg from the vertical is modelled as its vertical component ' + constants.INC_V + ' kN (with the L4 e and z_g) plus an entered CONSTANT M_z,Ed = ' + constants.INC_MZ_ED + ' kN.m = ' + constants.GAMMA_Q + ' x ' + constants.INC_MZ + ' (the simply supported minor-axis moment of the horizontal component ' + constants.INC_H + ' kN at 2.0 m, H a b / L = ' + constants.INC_MZ + ' kN.m characteristic, factored by gamma_Q because the tool\'s M_z input is the ULS design value M_z,Ed entered directly - corrected in the 20 Sep 2026 review; the first build entered the characteristic value). The constant M_z,Ed is the peak of the true triangular M_z diagram applied along the whole member: the tool takes C_mz = 1.0 (uniform minor-axis moment; js/checks/eurocode-checks.js Cmz = 1.0 in Eq 6.61/6.62 and in EN 1993-6 (A.1) whenever an M_z is imposed - the eigen-route C_mz override applies to the twist-induced phi.M_y diagram only, 20 Sep 2026), exact for the constant entered and conservative against the true triangular diagram; with torsion M_z,Ed = M_z + phi.M_y in Eq 6.62, (A.1) and the P385 3.1.2 / (6.1) cross-section checks (20 Sep 2026 torsion + N/Mz); no minor-axis shear or deflection is computed, and the torque of the horizontal component acting at z_g (H x z_g = ' + constants.INC_H + ' kN x D/2, e.g. 2.36 kN.m characteristic on the UB, 1.03 kN.m on the PFC) is not modelled. The case is titled as an approximation. Needed: a lateral load list with its own V_z / M_z diagrams, minor-axis deflection, and the torque of lateral loads at their height.'],
