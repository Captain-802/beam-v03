...
function runOne(cs, method) {
  ...
  if (method === 'full') {
    ...
    if (L.failed) { rec.MbRd = 0; rec.ltbFailed = stripHtml(L.err || ''); }
      ...
      rec.MbRd = num(L.MbRd);
      rec.stdBasis = /M.?cr.? method/i.test(rec.ltbBasis) ? 'Mcr route' : (L.channel ? 'channel kappa chain' : 'Mcr route');
    }
    rec.zg = num(L.zg);
  }
...
runs.forEach(r => {
  ...
  md.push(`| ${r.id} | ${mdEsc(r.section)} | ${mdEsc(r.variant + ': ' + r.owner)} | ${r.method} | ${r.verdict} | ${r.gov ? mdEsc(r.gov.name) + ' (' + fmtU(r.gov.val) + ')' : '-'} | ${fmt(r.Mcr, 1)} | ${fmt(r.C1, 3)} | ${fmt(r.MbRd, 1)} | ${fmtU(r.p385)} | ${fmtU(r.vt)} | ${fmtU(r.annexA)} | ${fmtU(r.boxT)} | ${fmt(r.deflRatio)} | ${mdEsc(msgs || '-')} |  |`);
});
md.push('');
...
const COVER = [
  ...
  ['load applied at angle on beam', 'L6', 'APPROXIMATED', 'The tool has no lateral (y-direction) load. A further 30 kN point load at 2.0 m inclined ' + constants.INC_DEG + ' deg from the vertical is modelled as its vertical component ' + constants.INC_V + ' kN (with the L4 e and z_g) plus an entered CONSTANT M_z = ' + constants.INC_MZ + ' kN.m = the simply supported minor-axis moment of the horizontal component ' + constants.INC_H + ' kN at 2.0 m (H a b / L). The constant M_z is the peak of the true triangular M_z diagram applied along the whole member (conservative for the cross-section checks, over-states C_mz), no minor-axis shear or deflection is computed, and the torque of the horizontal component acting at z_g (H x z_g) is not modelled. The case is titled as an approximation. Needed: a lateral load list with its own V_z / M_z diagrams, minor-axis deflection, and the torque of lateral loads at their height.'],
  ...
  ['WARPING RESTRAINED', 'R4', 'COVERED', 'The warping flag of both ends (phi\' = 0): the LTB eigen model applies it as a boundary condition (SN003a k_w = 0.5 type) and the torsion of the eccentric load is routed to the general warping-torsion FE with phi\' = 0 at both ends (the P385 closed forms assume warping free). The standard M_cr route keeps k_w = 1 (fork ends) and prints the flag as not applied - the standard-route M_cr of R4 therefore equals R2.'],
  ...
  ['DESTABLIZING LOADS', 'R6 (and L3t / L4 / L5..L7)', 'COVERED via z_g; the switch alone is REFUSED (blocked)', 'The modelled destabilising case is the load height z_g = +D/2 (L3t, L4 and their descendants; R7 is the stabilising twin). R6 ticks the "destabilising" switch with every z_g = 0: the eigen route BLOCKS this contradictory input (the switch is the BS 5950 x1.2 L_E device, the eigenvalue carries the height through z_g; PASS blocked, message printed - a refusal that keeps the run), the standard route applies L_E = 1.2 L (BS 5950 practice) and runs. Compare MasterSeries\' "destabilising" option against the standard run of R6 and the eigen run of L3t.'],
...
cov.push('- **Channel (PFC) on the standard route:** the design basis is the P385/P362 kappa chain (a lower bound independent of the moment shape, end flags and load height); the printed M_cr of a PFC standard run is the value behind that chain and is well below the eigen M_cr (explained in tests/batch/mcr-method-comparison.md).');
...
cov.push('');
cov.push('## Engine observations recorded while building the matrix (not corrected here)');
cov.push('');
...
// ---------------------------------------------------------------------------
{
  ...
  const SHORT = { L1: 'no torsion', L2: 'horizontal ecc. e = +40', L3: 'vertical ecc. z_g = +/-D/2', L4: 'e + z_g', L5: 'e + z_g + N', L6: 'L5 + inclined load (approx.)', L7: 'L6 + couple 40 kN.m', R1: 'fully restrained', R2: 'unrestrained (reference)', R3: 'restrained at 2, 4 m', R4: 'warping restrained', R5: 'warping free', R6: 'destabilising switch, z_g = 0', R7: 'stabilising z_g = -D/2', R8: 'L_E override 0.85 L', R9: 'top flange restrained (= full)', R10: 'bottom flange restrained', R11: 'partially restrained' };
  const rows = runs.map(r => `<tr><td class="id">${htmlEsc(r.id)}</td><td>${htmlEsc(r.section)}</td><td>${htmlEsc(r.variant + ': ' + (SHORT[r.variant] || r.owner))}</td><td>${htmlEsc(r.method)}</td><td class="v ${r.verdict === 'NOT VERIFIED' ? 'NV' : r.verdict}">${htmlEsc(r.verdict)}</td><td>${r.gov ? htmlEsc(r.gov.name) + ' (' + fmtU(r.gov.val) + ')' : '-'}</td><td class="n">${fmt(r.Mcr, 1)}</td><td class="n">${fmt(r.MbRd, 1)}</td><td class="n">${fmtU(r.annexA)}</td><td class="n">${fmt(r.deflRatio)}</td><td class="cmp"></td></tr>`).join('\n');
  ...
  <p>Method: eigen = FE eigenvalue M_cr; standard = SN003a / P385 closed forms; full = fully restrained; refused = not run. Annex A = EN 1993-6 LTB + torsion (99.000 = unbounded, M_y,Ed &ge; M_cr). Comparison column: MasterSeries figures.</p>
