...
test('pattern loading off reproduces the previous engine figures exactly (two-span, overhang with LTB, three-span standard route)', () => {
  ...
  const ref = {
    ...
    threeSpanStd: {over:{L:12,restraint:'ltb',mcrMethod:'standard',supports:[{pos:0,type:'pinned'},{pos:4,type:'pinned'},{pos:8,type:'pinned'},{pos:12,type:'pinned'}],loads:[{type:'trap',x1:0,x2:12,w1:5,w2:15,case:'Q'}],eccOn:true,za:0},
                   ...
                   // Re-baselined again 19 Sep 2026 (verification campaign, finding F1): the Serna quarter-point ordinates are read 1e-3 mm either side of the station (larger side of a jump), which moves this smooth diagram's C1 by 6e-7 (Mcr 558.9359 -> 558.9353, util 0.07776827 -> 0.07776831); forces and reactions unchanged
                   Mmax:-32.404166948376066, Vmax:48.606254141162886, dmax:-0.31315338210286303, utils:[0.06427191936520846,0.0643898001954815,0.07776831437232731,0.028183804389257675], R:[15737.504000043244,55778.13600018919,85778.13599952446,35737.50399959248], Mcr:558.9352582382098, MbRd:416.67570153618504},
  };
