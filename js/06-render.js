...
function render(){
  ...
  const upliftLines=(HD&&HD.rows&&HD.rows.length)
    ? HD.rows.map(u=>{
        if(u.level==='sls') return `<div class="note" style="margin-left:0"><b>Uplift at SLS only, support ${u.n} (x = ${g(u.pos/1000,2)} m):</b> R = &minus;${f1(Math.abs(u.RSls),2)} kN (SLS combination ${u.comboSls}); no ULS combination lifts this support, including the &gamma;<sub>G,inf</sub> companions with G at 1.0 (STR set B) and 0.9 (EQU set A) (advisory).</div>`;
