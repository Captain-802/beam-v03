...
**[beam-v03 addition, 19 Sep 2026; single-span scope: the automatic pattern-loading lines and `a.patterns` are gone - one span has nothing to pattern - and only the gamma_G,inf companion lines remain, with `a.companionNote` in place of `a.patterns.limitation`]** the load list ends with the automatic pattern-loading lines (`a.patterns`: segments, generated ULS/SLS cases numbered by the expanded lists, followed since the 19 Sep 2026 review fixes by the gamma_G,inf companion lines - `a.ulsCompanions`, one summary line and one line per companion `ULS Cn: <parent label> [gamma_G,inf = 1.0, STR set B]` / `[... = 0.9, EQU set A]`, reactions only - and the companion note `a.patterns.limitation`), and the reactions line is followed by the uplift rows (`c.holdDown.rows`: "Hold-down required" as a NOT VERIFIED row, "Hold-down provided" as an advisory row with the design force, or "Uplift ... OK"). Case numbers (`msbCaseIndex(combo, sls, a)`) index the analysed lists `a.ulsResults` / `a.slsResults`, so a generated pattern is "Load Case 3 (ULS: 1.35G + 1.5Q (Q on span 2 only))".
...
| `s<sub>s</sub>, c ; k<sub>F</sub>` | `s<sub>s</sub> = … (entered / default 0 / not entered: lower bound 0) [capped at h<sub>w</sub>]; c = … (d = … to the member end) ; k<sub>F</sub> = …` | k<sub>F</sub> | `Fig 6.1(a/b/c)` | station `ss, ssIn, ssDefault, ssCap, c, d, a`, `gov.kF`; the derivation prints `c.web.show` (worst evaluated station), headed `Worst station (NOT VERIFIED: s<sub>s</sub> not entered, lower bound 0)` when that station is NOT VERIFIED (review fix F3) |
...
| table, one row per station | x, station, type, s<sub>s</sub> (asterisk = not entered, lower bound 0), k<sub>F</sub>, l<sub>y</sub>, λ̄<sub>F</sub>, χ<sub>F</sub>, F<sub>Rd</sub>, F<sub>Ed</sub>, load case, F<sub>Ed</sub>/F<sub>Rd</sub>, (η<sub>2</sub>+0.8η<sub>1</sub>)/1.4, status (`governs` / `OK` / `Warning` / `NOT VERIFIED` for a blank-s<sub>s</sub> station failing at 0, which also prints its blocking message as a NOT VERIFIED row of the block); a stiffened station prints `stiffener declared - design stiffener separately (EN 1993-1-5 9.4)` with tag `advisory` | | | | `c.web.stations[]` |
...
| `M<sub>z,tot</sub> = M<sub>z.Ed</sub> + φ.M<sub>y.Ed</sub>` (replaces the row above when an M<sub>z</sub> is imposed; 20 Sep 2026 torsion + N/Mz) | `<MzImp> + <My> x <phi>e-3 (@ x) = <MzImp> + <MzMax> (imposed constant M_z.Ed + max coincident twist moment)` | `T.MzTot` kN.m | `P385 3.1.2` (hollow sections: `P385 3.1.2 / 5.2.1(3)`) | `T.MzImp`, `T.MzMax`, `T.MzTot`, the grid row of the largest φ.M<sub>y</sub> (`msbMzTwistRow`) |
...
| `C<sub>mz</sub> (EN 1993-6 A.1)` (only with an imposed M<sub>z</sub>, 20 Sep 2026) | `AN.CmzBasis` (`C_mz = 1.0: the imposed M_z is a constant diagram (psi = 1, Table B.3); the twist-induced part carries no proxy`) | `AN.Cmz` | `Table B.3` | `annexAEval` |
| `M<sub>y</sub>/M<sub>b.Rd</sub> + C<sub>mz</sub>M<sub>z</sub>/M<sub>z.Rk</sub> + k<sub>w</sub>k<sub>zw</sub>k<sub>α</sub>M<sub>w</sub>/M<sub>f.Rk</sub>` (only when `AN`, unrestrained path; label `M<sub>z,tot</sub>` and the note `(M_z,tot = <imposed> imposed + <twist> twist phi.M_y)` with an imposed M<sub>z</sub>; the `k = kw.kzw.kα` row then adds `k_zw = 1 − M_z,tot/M_z.Rk`; a channel adds the `(A.1) scope` basis row: the annex states (A.1) for I sections, applied on the SCI P385 basis [verify] - 20 Sep 2026 review) | `@ x = <AN.x/1000> m: <My>/<MbA> + <Cmz>x<Mz>/<MzR> + <kw>x<kzw>x<kAlpha>x<Mw>/<MfR>` | `AN.u` | OK / Warning (`BLOCKED` when `AN.kAlpha` is infinite) | `AN.{u, x, My, Mz, Mw, kw, kzw, kAlpha, Cmz, MbA, McrA, MzR, MfR, combo}` (EN 1993-6 Annex A / P385 6.2) |
| `Basis with N<sub>Ed</sub> / M<sub>z.Ed</sub>` (basis row, only with N<sub>Ed</sub> or an imposed M<sub>z</sub>; 20 Sep 2026 torsion + N/Mz) | `T.combinedBasis` verbatim - per path (`torsionCombinedBasis`): open sections `No expression in EN 1993-1-1 or EN 1993-6 combines N_Ed with warping torsion at member level; the verification is EN 1993-1-1 6.2.7(5)/(6.1) at the cross-section with all stresses [information only ...], cl 6.3.3 (6.61/6.62) ... [or: not evaluated for tension / does not apply to a fully restrained member without axial compression], and EN 1993-6 (A.1) with M_z,Ed = M_z + phi.M_y [or: not evaluated for a fully restrained member]`; hollow sections `EN 1993-1-1 6.2.7(7): warping neglected ...; EN 1993-6 (A.1) does not apply to a closed section` | | `EN 1993-1-1, EN 1993-6` | `T.combinedBasis`, `T.combinedActive` |
| `superposition of Eq 6.62 and (A.1) - not a Eurocode expression, information only` (class `ms-advrow`, only with N<sub>Ed</sub> > 0 and an open-section torsion solution) | `N_Ed/(χ_z N_Rk/γ_M1) + k_zy M_y.Ed/(χ_LT M_y.Rk/γ_M1) + k_zz M_z,tot/(M_z.Rk/γ_M1) + k_w k_zw k_α M_w/M_f.Rd = <u62> (Eq 6.62 with M_z,tot = <B.MzEd>) + <uw> (warping term of (A.1) @ x: k_w, k_zw, k_α [(no LTB check: M_cr unbounded) when fully restrained], M_w) =` | `T.superposition.u` | `information only` (never OK / Warning, never in `c.utils`; also in the advisory footer) | `T.superposition` (`torsionSuperposition`) |
...
| Label / formula | Substituted values | Result | Tag | Source |
|---|---|---|---|---|
| `In-span δ ≤ Span/<divisor>` | `<dmax> ≤ <span mm> / <divisor>` (+ `@ x = <dpos> m`; the multi-span `segment <start>–<end> m` suffix is N/A since the single-span scope) | `c.dmax` mm | OK / Warning | `c.dmax`, `c.span`, `c.divisor`, `c.dlimit`, `c.defOk`, `a.deflection.{start, end, dpos}`. Cantilever: label `Tip δ ≤ L/<divisor>` |

**Single-span scope (19 Sep 2026):** one span, so `a.deflection` is the one segment - span/`S.divisor` between two vertically held ends, L/`S.divisorCant` when an end is vertically free (cantilever or guided tip: the tip deflection relative to the held end), capped by `S.deflAbs`; `a.deflSegments` has one entry and the per-segment rows below are historical. Beam-v03 checked every support-to-support span and each overhang with its own length (`analyse()` keeps the worst in `a.deflection`). Since the 19 Sep 2026 gap closure the limit is per segment: span/`S.divisor` between supports, L/`S.divisorCant` (default 180, UK NA Table NA.2 cantilever row [verify]) for a cantilever segment, capped by the optional absolute limit `S.deflAbs`; the governing row prints the limit used (label `Tip &delta; &le; L/180` for a cantilever segment, `(&le; x mm)` when an absolute limit is entered) and a multi-segment member adds one row per segment (`a.deflSegments`, worst SLS combination named). MasterSeries' Def Limit pattern rows are covered by the automatic pattern combinations of the SLS set.
...
| extra cells **[beam-v03 addition]** appended only when present in `c.utils`: `V/Vpl` (`c.shearUtil`, always), `M-V` (`c.coex.u`), `Torsion` (`T.cross.u` or `T.torUtil`), `Yield 6.1` (`T.elastic.u`, the elastic yield criterion (6.1) with torsion when verdict-binding: Class 3, or an open section with N<sub>Ed</sub>; otherwise the cell `Yield 6.1 (info)` from `c.info`, outside Max and never red - 20 Sep 2026 review), `V+T` (`T.vtUtil`), `LTB+T` (`AN.u`) | | `c.utils[].name/val`, `c.info` |
...
| extra cells as above (`V/Vpl`, `MA/Mc` = `c.momUtil`, `LTB` = `c.ltbUtil`, `M-V`, `M-V-N`, `N_b.T`, `F/F_Rd`, `Web 7.2`, `Torsion`, `Yield 6.1` / `Yield 6.1 (info)`, `V+T`, `LTB+T`) **[beam-v03 addition]** | | `c.utils`, `c.info` |
...
| Local / Moment Capacity block, after the plastic N-M-M<sub>z</sub> interaction rows (20 Sep 2026 torsion + N/Mz, every torsion case): `Elastic yield criterion with torsion (cl 6.2.7(5), Eq 6.1) @ x m, <point>` with the station data (M<sub>y</sub>, V, φ, φ′, φ″, φ‴ - or T for a box), `M_z,tot = M_z.Ed + φM_y = a + b = c` (or `M_z = φM_y`), the governing point's stress build-up term by term (`msbElasticTerms`: P1 tip `N/A + M_y/W_el.y + M_z,tot/W_el.z + EW_n0φ″`, `Gt_fφ′`; P1b channel flange at W<sub>n</sub> = 0 `ES_w1φ‴/t_f` and the flange shear flow `Vs_1t_f(D−t_f)/(2I_yt_f)`; P2 junction flange `M_y(D/2−t_f/2)/I_y`, `M_z,tot y_w/I_z`, `EW_n2φ″` (channel), `VS_f/(2I_yt_f)` (I/H) or `VS_f/(I_yt_f)` (channel), `ES_w1φ‴/t_f` (I/H) or `ES_w2φ‴/t_f` (channel); P3 junction web `VS_f/(I_yt_w)`, `Gt_wφ′`, `ES_w2φ‴/t_w`; P4 web mid-depth `VS_max/(I_yt_w)`, `ES_w3φ‴/t_w`; box corner `T_Ed/W_t + VQ_c/(I_yt)`, web mid-depth `T_Ed/W_t + VQ_m/(I_yt)`, flange mid-width `T_Ed/W_t`) and `(σ_x/f_y)² + 3(τ/f_y)² =`; result `T.elastic.u`; tag `OK / Warning 6.2.7(5)` when verdict-binding, `≤ 1 information 6.2.7(5)` / `> 1 information 6.2.7(5)` otherwise (20 Sep 2026 review). Then the indented `ms-pts` row `σ_x, τ, (6.1) at the n section points` (every point's sums and value, the governing one marked, plus y<sub>w</sub>, the channel s<sub>1</sub> / y<sub>1</sub>, the box Q<sub>c</sub> / Q<sub>m</sub>) and the `ms-basis` row `Elastic verification (6.1): verdict-binding` / `information only` = `T.elastic.bindingBasis` (`elasticBindingPolicy`: Class 3, or an open Class 1/2 section with N<sub>Ed</sub>, binds; Class 1/2 without N<sub>Ed</sub> and Class 1/2 hollow sections are information - 6.2.7(5) "may be applied", 6.2.7(6) / 6.2.7(7)-(9) plastic route) | `T.elastic` | `msbElasticRows`, `T.elastic.{u, x, point, combo, sigmaN, sigmaMy, sigmaMz, sigmaW, sigmaX, tauV, tauT, tauW, tau, fy, points, MzImp, MzTwist, MzTot, geom, binding, bindingBasis}` |
| Equivalent Uniform Moment Factors `C_mz = Max(0.6+0.4ψ, 0.4)` with torsion: `M_z.Ed = <imposed> imposed + <twist> twist φ.M_y = <total>; ψ = 1 for the imposed constant M_z, the twist part taken at C_mz = 1.0 (Table B.3 upper bound, conservative)`; Buckling Resistance `U_M.z` row `(<imposed> imposed + <twist> twist φ.M_y = <total>) / M_z.Rk` (20 Sep 2026 torsion + N/Mz, review wording) | `B.MzTwist > 0` | `B.MzImp`, `B.MzTwist`, `B.MzEd` (annexB2: M<sub>z,Ed</sub> = imposed + max\|φM<sub>y</sub>\| of the combination's torsion solution) |
| Verdict footer: the engine advisories, including `ADVISORY - elastic yield criterion (6.1) with torsion ... = x at x = ... (point): first yield is reached under the design actions (information only, not a utilisation: ...)` when the informational (6.1) exceeds 1, and the superposition text; a note that already opens with `ADVISORY - ` is not prefixed twice | `c.advisory` | |
