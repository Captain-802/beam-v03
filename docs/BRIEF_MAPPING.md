...
| Label / formula | Substituted values | Result | Tag | Source |
|---|---|---|---|---|
| `V<sub>z.Ed</sub>/V<sub>pl.z.Rd</sub>` | `0 / <VplZ> =` | `0` | `Low Shear` | V<sub>z.Ed</sub> NOT AVAILABLE (no minor-axis shear in the model; print 0 with the note "no minor-axis shear in the single-plane model"). V<sub>pl.z.Rd</sub> DERIVE per EN 1993-1-1 6.2.6(3)(c) for rolled I/H, load parallel to flanges: `Av,z = sec.A*100 − c.hw*sec.tw` mm<sup>2</sup>, `VplZ = Av,z*c.fy/(√3)/1000` kN (UB 161: 2568.7 kN vs MasterSeries 2541.93); for boxes `Av,z = sec.A*100*sec.B/(sec.D+sec.B)`; channels: omit the line (Av,z rule for channels is not in the check code) |
| `M<sub>c.z.Rd</sub> = f<sub>y</sub>.W<sub>pl.z</sub>/γ<sub>M0</sub>` (or `W<sub>el.z</sub>`) | `<fy> x <Sy or Zy>/1` | kN.m | | `B.Mcz` when `B` exists, else DERIVE `(c.cl.cls<=2 ? sec.Sy : sec.Zy)*c.fy/1e3` |
...
| `M<sub>y.Ed</sub>/M<sub>b.Rd</sub>` | `<Mx> / <MbMcr>` | DERIVE `c.Mx/LT.MbMcr` | OK / Warning | today's closed-form verdict uses `LT.MbRd = MbSimp` (P362 Expn 6.55 simplified slenderness) and only "rescues" with the M<sub>cr</sub> route; for the MasterSeries-format brief the M<sub>cr</sub> route is the printed basis. **Decision needed** (section 11): make the standard option's `LT.MbRd = LT.MbMcr` so that `c.ltbUtil` equals the printed ratio; until then print `c.ltbUtil` and, if it differs, a **[beam-v03 addition]** line `λ<sub>LT</sub> (P362 6.55 simplified) = <lamLTsimp>, M<sub>b.Rd</sub> = <MbSimp>` with `LT.lamLTsimp`, `LT.chiModS`, `LT.MbSimp` |
...
| Open sections (`T.p385`): `φ<sub>max</sub> (ULS)` | `L/a = <T.X>; fork ends, warping free (Cases 3/4/10)` | `T.phiUmax` rad, ° | | `T.phiUmax`, `T.X` |
...
| `End torques T<sub>t</sub>` | `x = 0 / x = L` | `|T.TtEnds[0]| / |T.TtEnds[1]|` kN.m | | `T.TtEnds` |
...
1. **Design basis of the standard option.** Print (and verify with) the SN003a M<sub>cr</sub> route, `LT.MbMcr`, rather than the P362 simplified slenderness `LT.MbSimp` that the closed-form function currently uses as its design basis. Recommended: when `S.mcrMethod === 'sn003a'`, set `LT.MbRd = LT.MbMcr` and `c.ltbUtil = c.Mx/LT.MbMcr`, keeping the simplified route as an informational line. Otherwise the printed ratio and the verdict can disagree.
